...
// Required GENERAL_INFO fields for on_cancel (no ROOM_RENT_CAP)
...
export function onCancel(payload: any): validationOutput {
  ...
  // Validate provider descriptor images size_type
  if (!provider) {
    results.push({
      valid: false,
      code: 30000,
      description: "Provider is missing in on_cancel",
    });
  } else {
  ...
  if (!Array.isArray(items) || items.length === 0) {
    results.push({
      valid: false,
      code: 30000,
      description: "Items array is missing or empty in on_cancel",
    });
  } else {
  ...
  if (!Array.isArray(cancellationTerms) || cancellationTerms.length === 0) {
    results.push({
      valid: false,
      code: 30000,
      description: "Cancellation_terms is missing or empty in on_cancel",
    });
  } else {
