...
// Required GENERAL_INFO fields for on_confirm (includes ROOM_RENT_CAP)
...
export function onConfirm(payload: any): validationOutput {
  ...
  RedisService.setKey(
    `${transaction_id}:health:onConfirmItems`,
  ...
  if (!provider) {
    results.push({
      valid: false,
      code: 30000,
      description: "Provider is missing in on_confirm",
    });
  } else {
  ...
  if (!Array.isArray(items) || items.length === 0) {
    results.push({
      valid: false,
      code: 30000,
      description: "Items array is missing or empty in on_confirm",
    });
  } else {
  ...
  if (!Array.isArray(cancellationTerms) || cancellationTerms.length === 0) {
    results.push({
      valid: false,
      code: 30000,
      description: "Cancellation_terms is missing or empty in on_confirm",
    });
  } else {
