...
export class MockFamilyInformationFormClass extends MockAction {
	name(): string {
		return "family_information_form";
	}
	get description(): string {
		return "Mock for family_information_form";
	}
	...
	async validate(
		...
		const formLink = sessionData["family_information_form"];
		if (!formLink) {
	...
	override async __forceSaveData(
		...
		return {
			...sessionData,
			family_information_form: resolveFormActions(formLink, formData),
		};
	}
	...
	get saveData(): saveType {
		return { "save-data": { family_information_form: "family_information_form" } };
	}
	get defaultData(): any {
		return {};
	}
