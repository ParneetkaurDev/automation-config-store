...
export class MockPersonalDetailsFormClass extends MockAction {
	name(): string {
		return "personal_details_form";
	}
	get description(): string {
		return "Mock for personal_details_form";
	}
	...
	async validate(
		...
		const formLink = sessionData["personal_details_form"];
		if (!formLink) {
 ...
 async __forceSaveData(
		...
		return {
			...sessionData,
			personal_details_form: resolveFormActions(formLink, formData),
		};
	}
	...
	get saveData(): saveType {
		return { "save-data": { personal_details_form: "personal_details_form" } };
	}
	get defaultData(): any {
		return {};
	}
