...
export class MockPanDetailsFormClass extends MockAction {
	name(): string {
		return "pan_details_form";
	}
	get description(): string {
		return "Mock for pan_details_form";
	}
	...
	async validate(
		...
		const formLink = sessionData["pan_details_form"];
		if (!formLink) {
	 ...
	 async __forceSaveData(
		...
		return {
			...sessionData,
			pan_details_form: resolveFormActions(formLink, formData),
		};
	}
	...
	get saveData(): saveType {
		return { "save-data": { pan_details_form: "pan_details_form" } };
	}
	get defaultData(): any {
		return {};
	}
