...
export class MockNomineeDetailsFormClass extends MockAction {
	name(): string {
		return "nominee_details_form";
	}
	get description(): string {
		return "Mock for nominee_details_form";
	}
	...
	async validate(
		...
		const formLink = sessionData["nominee_details_form"];
		if (!formLink) {
	...
	override async __forceSaveData(
		...
		return {
			...sessionData,
			nominee_details_form: resolveFormActions(formLink, formData),
		};
	}
	...
	get saveData(): saveType {
		return { "save-data": { nominee_details_form: "nominee_details_form" } };
	}
	get defaultData(): any {
		return {};
	}
