...
export async function on_select_redemptionDefaultGenerator(
    existingPayload: any,
    sessionData: SessionData
) {
    console.log("=== on_select_redemption Generator Start ===");
    ...
    // // Inject fulfillment from session if available
    ...
    // Inject payments from session
    if (sessionData.payments && existingPayload.message?.order?.payments) {
        existingPayload.message.order.payments = sessionData.payments;
    }

    console.log("=== on_select_redemption Generator End ===");
