...
export function validateFormHtml(html: string): ValidationResult {
	try {
		...
		// --- Required business fields for Investor Details Form ---

		// Name
		const hasName = fields.some((f) => f.name === "name");

		// DOB
		const hasDob = fields.some((f) => f.name === "dob");

		// Gender
		const hasGender = fields.some((f) => f.name === "gender");

		// Marital Status
		const hasMaritalStatus = fields.some((f) => f.name === "marital_status");

		// Occupation
		const hasOccupation = fields.some((f) => f.name === "occupation");

		// Source of Wealth
		const hasSourceOfWealth = fields.some((f) => f.name === "source_of_wealth");

		// Income Range
		const hasIncomeRange = fields.some((f) => f.name === "income_range");

		// Validate all required fields
		if (!hasName) errors.push('Required "name" field not found.');
		if (!hasDob) errors.push('Required "dob" field not found.');
		if (!hasGender) errors.push('Required "gender" field not found.');
		// if (!hasMaritalStatus) errors.push('Required "marital_status" field not found.');
		// if (!hasOccupation) errors.push('Required "occupation" field not found.');
		// if (!hasSourceOfWealth) errors.push('Required "source_of_wealth" field not found.');
		// if (!hasIncomeRange) errors.push('Required "income_range" field not found.');
