export async function onStatusUnsolicitedGenerator(existingPayload: any, sessionData: any) {
  ...
  const submission_id = sessionData.Ekyc_details_verification_status;;
  ...
  if (existingPayload.message?.order?.items?.[0]) {
    ...
    if (item.xinput?.form) {
      ...
      item.xinput.form.id = "Ekyc_details_verification_status";
    }
    ...
    if (item.xinput?.form_response) {
      ...
      if (submission_id) {
        item.xinput.form_response.submission_id = sessionData.Ekyc_details_form;;
      }
    }
  }
