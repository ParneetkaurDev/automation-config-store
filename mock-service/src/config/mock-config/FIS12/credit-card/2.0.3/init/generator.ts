...
export async function initDefaultGenerator(existingPayload: any, sessionData: any) {
  ...
  if (existingPayload.message?.order?.items?.[0]?.xinput?.form) {
    // Use form ID from session data (saved by on_select) or generate new one
    const formId = sessionData.form_id || crypto.randomUUID();
  ...
  if (existingPayload.message?.order?.items?.[0]?.xinput?.form_response) {
    ...
    if (submission_id) {
      ...
      existingPayload.message.order.items[0].xinput.form_response.submission_id = `submission_${crypto.randomUUID()}`;
    }
